...
async def test_context_variables_propagation():
...
async def test_task_groups_with_exceptions():
...
async def test_gather_exception_handling():
...
async def test_semaphore_and_locks():
...
async def test_wait_for_behavior():
...
async def test_as_completed_behavior():
...
async def test_shield_edge_cases():
...
@pytest.mark.py311_plus
async def test_nested_decorators_with_asyncio_features():
